...
public class GoogleAuthInfo {
  ...
  public GoogleAuthInfo(final OtpInfo info, final String accountName,
                        final String issuer) {
    _info = info;
  ...
  public OtpInfo getOtpInfo() { return _info; }
  ...
  public Uri getUri() {
    ...
    if (_info instanceof TotpInfo) {
      ...
      builder.appendQueryParameter(
          "period", Integer.toString(((TotpInfo)_info).getPeriod()));
    } else if (_info instanceof HotpInfo) {
      builder.authority("hotp");
      builder.appendQueryParameter(
          "counter", Long.toString(((HotpInfo)_info).getCounter()));
    } else {
    ...
    builder.appendQueryParameter("secret",
                                 new String(Base32.encode(_info.getSecret())));
  ...
  public static GoogleAuthInfo parseUri(final String s)
      throws GoogleAuthInfoException {
    Uri uri = Uri.parse(s);
    if (uri == null) {
  ...
  public static GoogleAuthInfo parseUri(final Uri uri)
      throws GoogleAuthInfoException {
    ...
    try {
      ...
      default:
        throw new GoogleAuthInfoException(
            String.format("unsupported otp type: %s", type));
      }
