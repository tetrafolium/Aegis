...
public class AndOtpImporter extends DatabaseImporter {
  ...
  public AndOtpImporter(final Context context) { super(context); }
  ...
  public State read(final FileReader reader) throws DatabaseImporterException {
    ...
    try {
      ...
      // andOTP doesn't have a proper way to indicate whether a file is
      // encrypted so, if we can't parse it as JSON, we'll have to assume it is
  ...
  public static class EncryptedState extends DatabaseImporter.State {
    ...
    private DecryptedState decrypt(final char[] password,
      ...
      try {
        ...
        if (oldFormat) {
          ...
          if (iterations < 1 || iterations > MAX_ITERATIONS) {
            throw new DatabaseImporterException(String.format(
                "Invalid number of iterations for PBKDF: %d", iterations));
          }
        ...
        byte[] tag =
            Arrays.copyOfRange(_data, _data.length - TAG_SIZE, _data.length);
        ...
        CryptResult result = CryptoUtils.decrypt(_data, offset + NONCE_SIZE,
                                                 len, cipher, params);
  ...
  public static class DecryptedState extends DatabaseImporter.State {
    ...
    private static DatabaseEntry convertEntry(final JSONObject obj)
      ...
      } catch (DatabaseImporterException | Base32Exception | OtpInfoException |
               JSONException e) {
