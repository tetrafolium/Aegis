...
public class KeyStoreHandle {
  ...
  public SecretKey generateKey(final String id) throws KeyStoreHandleException {
    if (!isSupported()) {
      throw new KeyStoreHandleException(
          "Symmetric KeyStore keys are not supported in this version of Android");
    }

    try {
      KeyGenerator generator =
          KeyGenerator.getInstance(KeyProperties.KEY_ALGORITHM_AES, STORE_NAME);
      generator.init(
          new KeyGenParameterSpec
              .Builder(id, KeyProperties.PURPOSE_ENCRYPT |
  ...
  private static boolean isKeyPermanentlyInvalidated(final SecretKey key) {
    ...
    try {
      ...
      // apparently KitKat doesn't like KeyPermanentlyInvalidatedException, even
      // when guarded with a version check it will throw a java.lang.VerifyError
      // when its listed in a 'catch' statement so instead, check for it here
