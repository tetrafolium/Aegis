...
public abstract class Slot extends UUIDMap.Value {
  ...
  protected Slot() { super(); }

  protected Slot(final UUID uuid, final byte[] key,
  ...
  /**
   * Decrypts the encrypted master key in this slot using the given cipher and
   * returns it.
   * @throws SlotException if a generic crypto operation error occurred.
   * @throws SlotIntegrityException if an error occurred while verifying the
   *     integrity of the slot.
   */
  public MasterKey getKey(final Cipher cipher)
      throws SlotException, SlotIntegrityException {
    try {
      CryptResult res = CryptoUtils.decrypt(_encryptedMasterKey, cipher,
                                            _encryptedMasterKeyParams);
  ...
  /**
   * Encrypts the given master key using the given cipher and stores the result
   * in this slot.
   * @throws SlotException if a generic crypto operation error occurred.
   */
  ...
  public Cipher createDecryptCipher(final SecretKey key) throws SlotException {
    try {
      return CryptoUtils.createDecryptCipher(
          key, _encryptedMasterKeyParams.getNonce());
  ...
  public static Slot fromJson(final JSONObject obj) throws SlotException {
    ...
    try {
      ...
      CryptParameters keyParams =
          CryptParameters.fromJson(obj.getJSONObject("key_params"));
      ...
      case Slot.TYPE_DERIVED:
        SCryptParameters scryptParams = new SCryptParameters(
            obj.getInt("n"), obj.getInt("r"), obj.getInt("p"),
