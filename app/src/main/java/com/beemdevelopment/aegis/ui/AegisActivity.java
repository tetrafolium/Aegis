...
public abstract class AegisActivity
    extends AppCompatActivity implements AegisApplication.LockListener {
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    // apply a dirty hack to make progress bars work even if animations are
    // disabled
  ...
  protected AegisApplication getApp() { return _app; }

  protected Preferences getPreferences() { return _app.getPreferences(); }
   ...
   */
  protected boolean isOpen() { return _resumed; }

  /**
   * Reports whether this Activity instance has become an orphan. This can
   * happen if the vault was locked by an external trigger while the Activity
   * was still open.
   */
  private boolean isOrphan() {
    return !(this instanceof MainActivity) &&
        _app.getDatabaseManager().isLocked();
  }

  private void setGlobalAnimationDurationScale() {
    float durationScale = Settings.Global.getFloat(
        getContentResolver(), Settings.Global.ANIMATOR_DURATION_SCALE, 0);
    if (durationScale != 1) {
      try {
        ValueAnimator.class.getMethod("setDurationScale", float.class)
            .invoke(null, 1f);
      } catch (Throwable t) {
        Toast.makeText(this, R.string.progressbar_error, Toast.LENGTH_SHORT)
            .show();
      }
    }
  }

  protected Theme getCurrentTheme() { return _currentTheme; }
}
