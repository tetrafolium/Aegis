...
public class IntroActivity
	extends AppIntro2 implements DerivationTask.Callback {
...
protected void onCreate(final Bundle savedInstanceState) {
	...
	homeSliderPage.setTitleColor(
		getResources().getColor(R.color.primary_text_dark));
	homeSliderPage.setDescription(getString(R.string.app_description));
	homeSliderPage.setDescColor(
		getResources().getColor(R.color.primary_text_dark));
	...
	_authenticationSlide.setBgColor(
	...
	_authenticatedSlide.setBgColor(
		getResources().getColor(R.color.colorSecondary));
	...
	endSliderPage.setDescription(
		getString(R.string.setup_completed_description));
...
@Override
public void onSlideChanged(final Fragment oldFragment,
                           final Fragment newFragment) {
	Intent intent = getIntent();
	int cryptType = intent.getIntExtra(
		"cryptType", CustomAuthenticationSlide.CRYPT_TYPE_INVALID);

	if (newFragment == _endSlide &&
	    cryptType != CustomAuthenticationSlide.CRYPT_TYPE_NONE) {
