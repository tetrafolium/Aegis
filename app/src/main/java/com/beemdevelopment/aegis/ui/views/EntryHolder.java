...
public class EntryHolder extends RecyclerView.ViewHolder {
  ...
  public EntryHolder(final View view) {
    ...
    _progressBar.getProgressDrawable().setColorFilter(primaryColorId,
                                                      PorterDuff.Mode.SRC_IN);
    _view.setBackground(
        _view.getContext().getResources().getDrawable(R.color.card_background));
  ...
  public void setData(final DatabaseEntry entry, final boolean showAccountName,
                      final boolean showProgress, final boolean hidden,
                      final boolean dimmed) {
    _entry = entry;
    _hidden = hidden;

    // only show the progress bar if there is no uniform period and the entry
    // type is TotpInfo
    setShowProgress(showProgress);

    // only show the button if this entry is of type HotpInfo
    _buttonRefresh.setVisibility(
        entry.getInfo() instanceof HotpInfo ? View.VISIBLE : View.GONE);
  ...
  public void loadIcon(final Fragment fragment) {
    if (_entry.hasIcon()) {
      ...
      TextDrawable drawable = TextDrawableHelper.generate(
          _entry.getIssuer(), _entry.getName(), _profileDrawable);
      _profileDrawable.setImageDrawable(drawable);
    }
  }

  public ImageView getIconView() { return _profileDrawable; }
  ...
  public void destroy() { _refresher.destroy(); }

  public void startRefreshLoop() { _refresher.start(); }

  public void stopRefreshLoop() { _refresher.stop(); }
  ...
  public void dim() { animateAlphaTo(DIMMED_ALPHA); }

  public void highlight() { animateAlphaTo(DEFAULT_ALPHA); }
  ...
  public boolean isCodeHidden() { return _hidden; }
}
