...
public class AuthActivity extends AegisActivity
    implements FingerprintUiHelper.Callback, SlotListTask.Callback {
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    // only show the fingerprint controls if the api version is new enough,
    // permission is granted, a scanner is found and a fingerprint slot is found
    if (_slots.has(FingerprintSlot.class) && FingerprintHelper.isSupported() &&
        FingerprintHelper.isAvailable(this)) {
      boolean invalidated = false;
      FingerprintManager manager = FingerprintHelper.getManager(this);

      try {
        // find a fingerprint slot with an id that matches an alias in the
        // keystore
        for (FingerprintSlot slot : _slots.findAll(FingerprintSlot.class)) {
          ...
          if (handle.containsKey(id)) {
            ...
            _fingerHelper = new FingerprintUiHelper(manager, imgFingerprint,
                                                    textFingerprint, this);
      ...
      // display a help message if a matching invalidated keystore entry was
      // found
      if (invalidated) {
    ...
    Dialogs.showSecureDialog(
        new AlertDialog.Builder(this)
            ...
            .setPositiveButton(android.R.string.ok,
                               (dialog, which) -> selectPassword())
            .create());
  }

  private <T extends Slot> void trySlots(final Class<T> type,
                                         final Object obj) {
  ...
  public void onTaskFinished(final SlotListTask.Result result) {
    if (result != null) {
      ...
      Intent intent = new Intent();
      intent.putExtra("creds",
                      new DatabaseFileCredentials(result.getKey(), _slots));
