...
protected void onCreate(final Bundle savedInstanceState) {
	...
	List<ImportEntry> entries =
		(ArrayList<ImportEntry>)intent.getSerializableExtra("entries");
	List<DatabaseImporterEntryException> errors =
		(ArrayList<DatabaseImporterEntryException>)intent.getSerializableExtra(
			"errors");
...
private void
showErrorDialog(final List<DatabaseImporterEntryException> errors) {
	Dialogs.showSecureDialog(
		...
		.setNeutralButton(
			...
			(dialog, which)->showDetailedErrorDialog(errors))
		.create());
}

private void
showDetailedErrorDialog(final List<DatabaseImporterEntryException> errors) {
	...
	Dialogs.showSecureDialog(
		...
		.setNeutralButton(android.R.string.copy,
			...
			ClipboardManager clipboard =
				(ClipboardManager)this.getSystemService(
					Context.CLIPBOARD_SERVICE);
			ClipData clip = ClipData.newPlainText(
				"text/plain", message);
			clipboard.setPrimaryClip(clip);
			Toast
			.makeText(this, R.string.errors_copied,
			          Toast.LENGTH_SHORT)
			.show();
		})
		.create());
