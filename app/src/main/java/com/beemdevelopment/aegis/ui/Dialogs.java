...
public static void
showDeleteEntryDialog(final Activity activity,
                      final DialogInterface.OnClickListener onDelete) {
	showSecureDialog(
...
public static void
showDiscardDialog(final Activity activity,
                  final DialogInterface.OnClickListener onSave,
                  final DialogInterface.OnClickListener onDiscard) {
...
public static void
showSetPasswordDialog(final Activity activity,
                      final Dialogs.SlotListener listener) {
	View view =
	...
	EditText textPasswordConfirm =
		view.findViewById(R.id.text_password_confirm);
	...
	dialog.setOnShowListener(d->{
			...
			button.setOnClickListener(v->{
				if (!EditTextHelper.areEditTextsEqual(textPassword,
				                                      textPasswordConfirm)) {
				        return;
				}
	...
	TextWatcher watcher = new TextWatcher() {
		public void onTextChanged(final CharSequence c, final int start,
		                          final int before, final int count) {
			boolean equal =
				EditTextHelper.areEditTextsEqual(textPassword, textPasswordConfirm);
			buttonOK.get().setEnabled(equal);
		}

		public void beforeTextChanged(final CharSequence c, final int start,
...
private static void showTextInputDialog(final Context context,
                                        final @StringRes int titleId,
                                        final @StringRes int hintId,
                                        final TextInputListener listener,
                                        final boolean isSecret) {
	View view =
		LayoutInflater.from(context).inflate(R.layout.dialog_text_input, null);
	...
	AlertDialog dialog =
		...
		.setPositiveButton(android.R.string.ok,
		                   (dialog1, which)->{
			char[] text =
				EditTextHelper.getEditTextChars(input);
...
public static void showTextInputDialog(final Context context,
                                       final @StringRes int titleId,
                                       final @StringRes int hintId,
                                       final TextInputListener listener) {
...
public static void showPasswordInputDialog(final Context context,
                                           final TextInputListener listener) {
	showTextInputDialog(context, R.string.set_password, R.string.password,
	                    listener, true);
}

public static void
showNumberPickerDialog(final Activity activity,
                       final NumberInputListener listener) {
	...
	numberPicker.setValue(
		new Preferences(activity.getApplicationContext()).getTapToRevealTime());
...
public static void
showFingerprintDialog(final Activity activity,
                      final Dialogs.SlotListener listener) {
	View view =
		activity.getLayoutInflater().inflate(R.layout.dialog_fingerprint, null);
	...
	try {
		...
		SecretKey key =
			new KeyStoreHandle().generateKey(slot.getUUID().toString());
	...
	AlertDialog dialog =
		...
		.setView(view)
		.setNegativeButton(android.R.string.cancel, null)
		.setOnDismissListener(d->{ helper.get().stopListening(); })
		.create();

	helper.set(new FingerprintUiHelper(manager, imgFingerprint, textFingerprint,
	                                   new FingerprintUiHelper.Callback() {
			@Override
			public void onAuthenticated() {
			        listener.onSlotResult(
					slot, obj.getCipher());
			        dialog.dismiss();
			}
...
public interface NumberInputListener { void onNumberInputResult(int number); }

public interface TextInputListener { void onTextInputResult(char[] text); }
