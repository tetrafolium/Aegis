...
public class AboutActivity extends AegisActivity {
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    btnChangelog.setOnClickListener(v -> {
      ChangelogDialog.create()
          ...
          .show(getSupportFragmentManager(), "CHANGELOG_DIALOG");
    });
  }
  ...
  private static String getCurrentAppVersion() {
    if (BuildConfig.DEBUG) {
      return String.format("%s-%s (%s)", BuildConfig.VERSION_NAME,
                           BuildConfig.GIT_HASH, BuildConfig.GIT_BRANCH);
    }
  ...
  private void openMail(final String mailaddress) {
    ...
    startActivity(
        Intent.createChooser(mailIntent, this.getString(R.string.email)));
  }

  private void showLicenseDialog() {
    String stylesheet = getString(R.string.custom_notices_format_style);
    int backgroundColorResource = getCurrentTheme() == Theme.AMOLED
                                      ? R.attr.cardBackgroundFocused
                                      : R.attr.cardBackground;
    String backgroundColor = String.format(
        "%06X", (0xFFFFFF & ThemeHelper.getThemeColor(backgroundColorResource,
                                                      getTheme())));
    String textColor = String.format(
        "%06X",
        (0xFFFFFF & ThemeHelper.getThemeColor(R.attr.primaryText, getTheme())));
    String licenseColor = String.format(
        "%06X", (0xFFFFFF & ThemeHelper.getThemeColor(
                                R.attr.cardBackgroundFocused, getTheme())));

    stylesheet =
        String.format(stylesheet, backgroundColor, textColor, licenseColor);
